...
.quick-field select {
    ...
    -moz-appearance: none;
    background-image: url("data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%235c6873' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpolyline points='6 9 12 15 18 9'%3E%3C/polyline%3E%3C/svg%3E");
...
.quick-field select:invalid,
.quick-field select option[value=""] {
    color: #8898a5;
}
    ...
    <div class="auto-container">
        <div class="quick-check-shell">
            ...
            <form class="quick-check-form" action="{{ route('quick_project_check.store') }}" method="POST" novalidate>
                ...
                @endforeach
                {{-- Agent slot: read from ?agent=N (1-7). Each unique form link targets a different WhatsApp number. --}}
                <input type="hidden" name="agent_slot" id="agent_slot" value="{{ old('agent_slot', max(1, min(7, (int) request('agent', 1)))) }}">
        ...
        Next / ඉදිරියට
                ...
                <div class="quick-step" data-step="2">
                    ...
                    <div class="quick-field">
                        <label>ඔබට දැනටමත් ඔබගේ house project එකට ඉඩමක් තිබේද?</label>
...
.land-size-options .quick-option {
    position: relative;
    display: flex;
    align-items: center;
    width: 100%;
...
.land-size-options .quick-option input {
    position: absolute;
    width: 1px;
    height: 1px;
    opacity: 0;
...
.land-size-options .quick-option span {
    display: block;
    width: 100%;
    color: #263642;
    line-height: 1.4;
...
.land-size-options .quick-option:hover {
    transform: none;
    background: #f7faf8;
    border-color: #7ea894;
    box-shadow: none;
...
.land-size-options .quick-option.is-selected {
    background: #fff8e9;
...
.land-size-options .quick-option.is-selected span {
    color: #000;
    font-weight: 700;
}
...
.land-size-options .quick-validation-message {
    grid-column: 1 / -1;
    margin-top: 2px;
}
...
@media only screen and (max-width: 767px) {
    ...
    .land-size-options .quick-option {
        ...
        padding: 15px 16px;
                    ...
                    <div class="quick-field">
                        <label for="project_location">Project location / ව්‍යාපෘතිය පිහිටි දිස්ත්‍රික්කය</label>
                        <select id="project_location" name="project_location" required>
                            <option value="" disabled {{ old('project_location') ? '' : 'selected' }}>Select District / දිස්ත්‍රික්කය තෝරන්න</option>
                            @foreach ([
                                ...
                                <option value="{{ $district }}" {{ old('project_location') === $district ? 'selected' : '' }}>{{ $district }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="quick-field">
                        <label>Approximate land size, if available / දළ වශයෙන් ඉඩමේ ප්‍රමාණය</label>
                        <div class="quick-options land-size-options">
                            @foreach ([
                                'Less than 6 Perch',
                                '6 – 10 Perch',
                                '10 – 20 Perch',
                ...
                <div class="quick-step" data-step="5">
                    ...
                    <div class="quick-field">
                        <label for="whatsapp_number">Phone Number / දුරකතන අංකය</label>
                        <input
                            type="tel"
                            id="whatsapp_number"
                            ...
                            pattern="0[0-9]{9}"
                            maxlength="10"
                            title="Enter a valid 10-digit Sri Lanka mobile number starting with 0 (e.g. 0771234567)"
                            oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,10);"
                            required
                        >
                        <small class="quick-field-hint">Enter 10 digits starting with 0 &mdash; e.g. <strong>0771234567</strong> &nbsp;|&nbsp; 0 දිය 10 ඉලක්කම් ඇතුළත් කරන්න</small>
                    </div>
